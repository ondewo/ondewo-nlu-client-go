...
# =====================================================================================
# ondewo-nlu-client-go - Makefile
#
# The ONDEWO NLU (Natural Language Understanding) gRPC client for go. Everything below the
# `api/` directory is GENERATED from the .proto files of the ondewo-nlu-api submodule by the
...
GO_MODULE_BASE_PATH=github.com/ondewo/ondewo-nlu-client-go
GO_MODULE_MAJOR=$(firstword $(subst ., ,$(ONDEWO_NLU_VERSION)))
...
GO_RELEASE_TAG=v${ONDEWO_NLU_VERSION}
...
install_precommit_hooks: ## Installs pre-commit hooks and sets them up for the ondewo-nlu-client-go repo
	pre-commit install
...
generate_ondewo_protos: ## Generate the go stubs from the .proto files of the ondewo-nlu-api submodule
	@[ -d ${ONDEWO_PROTOS_DIR} ] || { \
...
# stays invisible to `go get` once the release is tagged ("module .../ondewo-nlu-client-go@v7.1.0:
...
check_go_module_path: ## Assert go.mod and every import agree with the /vN suffix ONDEWO_NLU_VERSION requires
	@declared=`sed -n 's|^module[[:space:]][[:space:]]*\([^[:space:]]*\).*|\1|p' go.mod | head -1`; \
	if [ "$$declared" != "${GO_MODULE_PATH}" ]; then \
		echo "$(RED)[ERROR]$(NC) go.mod declares module '$$declared', but ONDEWO_NLU_VERSION=${ONDEWO_NLU_VERSION} requires '${GO_MODULE_PATH}'"; \
	...
	echo "$(GREEN)[SUCCESS]$(NC) go.mod declares $$declared, matching ONDEWO_NLU_VERSION=${ONDEWO_NLU_VERSION}"
...
# only noticed by whoever reads the release page afterwards.
	...
	if [ -z "$$notes" ]; then \
		echo "$(RED)[ERROR]$(NC) RELEASE.md has no '## Release ONDEWO NLU Go Client ${ONDEWO_NLU_VERSION}' entry"; \
