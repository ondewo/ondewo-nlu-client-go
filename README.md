...
```shell
go get github.com/ondewo/ondewo-nlu-client-go/v7@latest   ## or @v7.1.0 to pin an exact release
```
...
Releases are tagged twice on the same commit: with the ONDEWO release number (`7.1.0`), which is
what the [GitHub releases page](https://github.com/ondewo/ondewo-nlu-client-go/releases) lists and
what the rest of the ONDEWO client fleet uses, and with the `v`-prefixed spelling (`v7.1.0`), which
...
├── .github
│   └── workflows
│       ├── ci.yml                          <----- build, test and the credential-free publish rehearsal
│       └── release.yml                     <----- GitHub release + module proxy warm-up, on a v* tag push
├── CONTRIBUTING.md
...
* it must be spelled `v<semver>` — `7.1.0` alone is not a Go module version;
