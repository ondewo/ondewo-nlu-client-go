...
* **`api/` is generated and must never be edited by hand.** It is rewritten from scratch by
  ...
  [ondewo-nlu-api](https://github.com/ondewo/ondewo-nlu-api); a change to *how* the stubs are generated
  belongs in [ondewo-proto-compiler](https://github.com/ondewo/ondewo-proto-compiler).
* **`ondewo-nlu-api/` and `ondewo-proto-compiler/` are submodules.** Do not commit changes inside them
  from here. Change them in their own repository, release them, then move the pin in the `Makefile`
  (`ONDEWO_NLU_API_GIT_BRANCH`, `ONDEWO_PROTO_COMPILER_GIT_BRANCH`) and commit the new
  submodule commit.
...
`ONDEWO_NLU_VERSION` in the `Makefile` is the single source of truth, and it **must
match the ONDEWO NLU API in major and minor version**. Add a `RELEASE.md` entry for
